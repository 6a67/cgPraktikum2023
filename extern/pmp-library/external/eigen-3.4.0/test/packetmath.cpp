...
void packetmath_real()
{
    ...
    if (PacketTraits::HasRound || PacketTraits::HasCeil || PacketTraits::HasFloor || PacketTraits::HasRint)
    {
        ...
        Scalar val = numext::mini(Scalar(2) * static_cast<Scalar>(NumTraits<IntType>::highest()),
    ...
    if (PacketTraits::HasExp)
    {
        ...
        Scalar sout;
        Scalar special[]
            = {NumTraits<Scalar>::infinity(), -NumTraits<Scalar>::infinity(), NumTraits<Scalar>::quiet_NaN()};
        for (int i = 0; i < 3; ++i)
        {
            data1[0] = special[i];
