...
class FullReductionKernelFunctor
{
  public:
    ...
    FullReductionKernelFunctor(
        LocalAccessor scratch_, Evaluator evaluator_, EvaluatorPointerType final_output_, Index rng_, OpType op_)
    ...
    void operator()(cl::sycl::nd_item<1> itemID)
    {
        compute_reduction(itemID);
    }

    template <bool Vect = (Evaluator::ReducerTraits::PacketAccess & Evaluator::InputPacketAccess)>
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename ::Eigen::internal::enable_if<Vect>::type
...
struct PartialReductionKernel
{
    ...
    PartialReductionKernel(ScratchAcc scratch_,
        ...
        : scratch(scratch_), evaluator(evaluator_), output_accessor(output_accessor_), op(OpDef::get_op(op_)),
          ...
          reduce_elements_num_groups(reduce_elements_num_groups_), num_coeffs_to_preserve(num_coeffs_to_preserve_),
          num_coeffs_to_reduce(num_coeffs_to_reduce_)
    {
    }

    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
    element_wise_reduce(Index globalRId, Index globalPId, CoeffReturnType& accumulator)
    {
...
struct SecondStepPartialReduction
{
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE SecondStepPartialReduction(ScratchAccessor,
                                                                     ...
                                                                     const Index num_coeffs_to_preserve_,
                                                                     const Index num_coeffs_to_reduce_)
        : input_accessor(input_accessor_), output_accessor(output_accessor_), op(OpDef::get_op(op_)),
