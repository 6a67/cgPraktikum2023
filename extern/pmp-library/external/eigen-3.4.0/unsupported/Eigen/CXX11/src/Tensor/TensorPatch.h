...
struct TensorEvaluator<const TensorPatchOp<PatchDim, ArgType>, Device>
{
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType packet(Index index) const
    {
        ...
        Index patchIndices[2]
            = {indices[0] / m_outputStrides[output_stride_index], indices[1] / m_outputStrides[output_stride_index]};
        ...
        else
        {
            ...
            for (int i = 0; i < NumDims - 2; ++i)
            {
                ...
                const Index offsetIdx[2]
                    = {patchOffsets[0] / m_outputStrides[i + 1], patchOffsets[1] / m_outputStrides[i + 1]};
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorOpCost costPerCoeff(bool vectorized) const
    {
        const double compute_cost
            = NumDims
              * (TensorOpCost::DivCost<Index>() + TensorOpCost::MulCost<Index>() + 2 * TensorOpCost::AddCost<Index>());
