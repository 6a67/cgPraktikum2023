...
#undef EIGEN_THREAD_LOCAL
    ...
    && ((EIGEN_COMP_GNUC && EIGEN_GNUC_AT_LEAST(4, 8)) || __has_feature(cxx_thread_local)                              \
...
class ThreadLocal
{
  ...
  public:
    ...
    ThreadLocal(int capacity, Initialize initialize, Release release)
        : initialize_(std::move(initialize)), release_(std::move(release)), capacity_(capacity), data_(capacity_),
          ptr_(capacity_), filled_records_(0)
    {
