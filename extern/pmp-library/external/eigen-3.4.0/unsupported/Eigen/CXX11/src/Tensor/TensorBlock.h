...
struct TensorBlockResourceRequirements
{
    ...
    withShapeAndSize(TensorBlockShapeType shape_type, size_t size_in_bytes, TensorOpCost cost)
    {
...
class TensorMaterializedBlock
{
  public:
      ...
      private:
        ...
        Storage(Scalar* data,
            ...
            : m_data(data), m_dimensions(dimensions), m_strides(strides),
              m_materialized_in_output(materialized_in_output), m_strided_storage(strided_storage)
        {
        }
    ...
    prepareStorage(TensorBlockDesc& desc, TensorBlockScratch& scratch, bool allow_strided_storage = false)
    {
...
class TensorBlockAssignment
{
    ...
    struct InnerDimAssign
    {
        ...
        Run(Scalar* target, IndexType count, const Evaluator& eval, IndexType eval_offset)
        {
