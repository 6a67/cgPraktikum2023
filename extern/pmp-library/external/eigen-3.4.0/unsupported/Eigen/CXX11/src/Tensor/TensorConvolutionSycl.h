                              ...
                              convolution_type::CONV1D>
{
    ...
    EigenConvolutionKernel(Local_accessor local_acc_,
        ...
        : local_acc(local_acc_), device_evaluator(device_evaluator_), kernel_filter(kernel_filter_),
          buffer_acc(buffer_acc_), indexMapper(indexMapper_), kernelSize(kernelSize_), input_range(input_range_)
    {
    }
                              ...
                              convolution_type::CONV2D>
{
    ...
    EigenConvolutionKernel(Local_accessor local_acc_,
        ...
        : local_acc(local_acc_), device_evaluator(device_evaluator_), kernel_filter(kernel_filter_),
          buffer_acc(buffer_acc_), indexMapper(indexMapper_), kernel_size(kernel_size_), input_range(input_range_)
    {
    }
    ...
    void operator()(cl::sycl::nd_item<3> itemID)
    {
        ...
        for (size_t j = itemID.get_local_id(1); j < num_input[1]; j += itemID.get_local_range()[1])
        {
            ...
            for (size_t i = itemID.get_local_id(0); i < num_input[0]; i += itemID.get_local_range()[0])
            {
                ...
                const size_t tensor_index
                    = plane_input_offset
                      + indexMapper.mapGpuInputKernelToTensorInputOffset(i + input_offset[0], j + input_offset[1]);
