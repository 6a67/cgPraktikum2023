...
#undef CONTEXT_ARGS
    ...
    class EvalParallelContext
    {
      public:
        ...
        EvalParallelContext(const Self* self,
                            ...
                            DoneCallback done)
            : created_by_thread_id_(std::this_thread::get_id()), done_(this, std::move(done)), device_(self->m_device),
              ...
              rhs_(self->m_rightImpl,
                   ...
                   self->m_k_strides),
              buffer_(buffer), output_(buffer, tm), output_kernel_(self->m_output_kernel),
              tensor_contraction_params_(self->m_tensor_contraction_params), num_threads_(num_threads),
              shard_by_col_(shard_by_col), parallel_pack_(parallel_pack),
              parallelize_by_sharding_dim_only_(parallelize_by_sharding_dim_only), m_(tm), n_(tn), k_(tk), bm_(bm),
              bn_(bn), bk_(bk), nm_(nm), nn_(nn), nk_(nk), gm_(gm), gn_(gn), nm0_(nm0), nn0_(nn0),
    ...
    struct EvalShardedByInnerDimContext
    {
        EvalShardedByInnerDimContext(const Self* self,
            ...
            : evaluator(self), m_lhs_inner_dim_contiguous(evaluator->m_lhs_inner_dim_contiguous),
              m_rhs_inner_dim_contiguous(evaluator->m_rhs_inner_dim_contiguous),
              m_rhs_inner_dim_reordered(evaluator->m_rhs_inner_dim_reordered), result(result_buffer), m(m_size),
              n(n_size), k(k_size), done(std::move(done_callback)), buffer_size_bytes(m * n * sizeof(Scalar)),
              block_size(blockSize(k, num_threads)), num_blocks(divup<Index>(k, block_size)),
              num_pending_blocks(internal::convert_index<int>(num_blocks)),
              l0_ranges(divup<Index>(num_blocks, l0_size)), l0_state(l0_ranges), block_buffers(num_blocks)
        {
      ...
      private:
        ...
        template <int Alignment>
        EIGEN_STRONG_INLINE static void addAllToBuffer(
            size_t n, const Scalar* src_buf0, const Scalar* src_buf1, const Scalar* src_buf2, Scalar* dst_buf)
        {
        ...
        void evalAsync(Index start_block_idx, Index end_block_idx)
        {
            ...
            {
                Index mid_block_idx = (start_block_idx + end_block_idx) / 2;
                evaluator->m_device.enqueueNoNotification([this, mid_block_idx, end_block_idx]()
    ...
    TensorOpCost
    contractionCost(Index m, Index n, Index bm, Index bn, Index bk, bool shard_by_col, bool prepacked) const
    {
