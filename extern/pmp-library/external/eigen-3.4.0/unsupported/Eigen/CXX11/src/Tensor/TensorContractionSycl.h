...
struct ThreadProperties
{
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ThreadProperties(const StorageIndex linearLocalThreadId_,
        ...
        : linearLocalThreadId(linearLocalThreadId_), kGroupId(kGroupId_), mGroupOffset(mGroupOffset_),
          nGroupOffset(nGroupOffset_), kGroupOffset(kGroupOffset_), mLocalOffset(mLocalOffset_),
          nLocalOffset(nLocalOffset_), mGlobalOffset(mGlobalOffset_), nGlobalOffset(nGlobalOffset_), kSize(kSize_),
  ...
  public:
    ...
    struct TiledMemory
    {
        ...
        TiledMemory(const ThreadProperties<StorageIndex>&,
            ...
            : lhs_scratch_extract{}, rhs_scratch_extract{}, lhs_scratch_ptr_compute(lhs_scratch_extract.ptr),
        ...
        TiledMemory(const ThreadProperties<StorageIndex>& thread_properties,
            ...
            : lhs_scratch_extract{block_start_ptr}, rhs_scratch_extract{lhs_scratch_extract.ptr
              ...
              lhs_extract_index(local_id_extract<LHSBlockProperties, Properties::TileSizeDimM>(
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorContractionKernel(Scratch scratch_,
        ...
        : scratch(scratch_), lhs(lhs_), rhs(rhs_), out_res(out_res_), groupSizeM(groupSizeM_), groupSizeN(groupSizeN_),
          numTiles(numTiles_), triple_dim(triple_dim_)
    {
    }
    ...
    compute_block_per_tile(OutScalar* lhs_block_ptr, OutScalar* rhs_block_ptr, PacketReturnType* privateRes)
    {
        StorageIndex idx = 0;
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
    store(OutPtr* out_ptr, PacketReturnType* privateRes, StorageIndex mGlobalOffset, StorageIndex nGlobalOffset)
    {
                            ...
                            if (mOffset < triple_dim.M && (nGlobalOffset + nId < triple_dim.N))
                            {
                                out_ptr[mOffset + outputLD]
                                    = Eigen::TensorSycl::internal::PacketWrapper<PacketReturnType,
                                                                                 PacketSize>::scalarize(mId, privetOut);
                            }
                        }
                    }
    ...
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE std::pair<StorageIndex, StorageIndex>
    ...
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename ::Eigen::internal::enable_if<!need_sync>::type
    sync_thread(const cl::sycl::nd_item<1>&)
    {
        return;
    }
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
    compute_panel(const cl::sycl::nd_item<1>& itemID, ThreadProperties<StorageIndex>& thread_properties, OutPtr out_ptr)
    {
...
struct GeneralVectorTensor
{
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE GeneralVectorTensor(Scratch scratch_,
        ...
        : scratch(scratch_), vec(vec_), mat(mat_), out_res(out_res_), nonContractGroupSize(nonContractGroupSize_),
...
struct GeneralScalarContraction
{
    ...
    GeneralScalarContraction(
        Scratch scratch_, const LhsMapper lhs_, const RhsMapper rhs_, OutAccessor out_res_, const StorageIndex rng_)
                          ...
                          Eigen::SyclDevice>>
{
    ...
    template <TensorSycl::internal::contraction_type ct,
              ...
              typename RhsMapper>
    void
    launchTT(EvaluatorPointerType buffer, const LhsMapper& lhs, const RhsMapper& rhs, const TripleDim& triple_dim) const
    {
                                      ...
                                      StorageIndex C) const
    {
        ...
        const StorageIndex scratchSize = (Properties::WorkLoadPerThreadNC + CFactor) * Properties::LocalThreadSizeC
                                         * Properties::LocalThreadSizeNC;
        auto thread_range = cl::sycl::nd_range<1>(cl::sycl::range<1>(globalRange), cl::sycl::range<1>(localRange));
        if (cNumGroups > 1)
        {
    ...
    launchSC(EvaluatorPointerType buffer, const LhsMapper& lhs, const RhsMapper& rhs, StorageIndex K) const
    {
