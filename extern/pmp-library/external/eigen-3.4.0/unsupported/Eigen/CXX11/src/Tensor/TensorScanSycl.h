...
struct ScanParameters
{
    ...
    ScanParameters(index_t total_size_,
                   ...
                   index_t loop_range_)
        : total_size(total_size_), non_scan_size(non_scan_size_), scan_size(scan_size_),
          non_scan_stride(non_scan_stride_), scan_stride(scan_stride_), panel_threads(panel_threads_),
          group_threads(group_threads_), block_threads(block_threads_), elements_per_group(elements_per_group_),
...
struct ScanKernelFunctor
{
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ScanKernelFunctor(LocalAccessor scratch_,
        ...
        : scratch(scratch_), dev_eval(dev_eval_), out_accessor(out_accessor_), temp_accessor(temp_accessor_),
          scanParameters(scanParameters_), accumulator(accumulator_), inclusive(inclusive_)
    {
    }

    template <scan_step sst = stp, typename Input>
    typename ::Eigen::internal::enable_if<sst == scan_step::first, CoeffReturnType>::type
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void operator()(cl::sycl::nd_item<1> itemID)
    {
        ...
        for (Index loop_offset = 0; loop_offset < scanParameters.loop_range; loop_offset++)
        {
            ...
            {
                Index global_id = global_offset + next_elements;
                if ((((block_id * scanParameters.elements_per_block) + (ScanParameters<Index>::ScanPerThread * local_id)
                      + i)
                     < scanParameters.scan_size)
                    && (global_id < scanParameters.total_size))
                {
...
struct ScanAdjustmentKernelFunctor
{
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ScanAdjustmentKernelFunctor(LocalAccessor,
                                                                      ...
                                                                      const ScanParameters<Index> scanParameters_,
                                                                      Op accumulator_)
        : in_accessor(in_accessor_), out_accessor(out_accessor_), scanParameters(scanParameters_),
...
struct ScanInfo
{
    ...
    EIGEN_STRONG_INLINE ScanInfo(const Index& total_size_,
        ...
        : total_size(total_size_), scan_size(scan_size_), panel_size(panel_size_), non_scan_size(non_scan_size_),
          scan_stride(scan_stride_), non_scan_stride(non_scan_stride_), dev(dev_)
    {
        // must be power of 2
