...
#pragma unroll
                ...
                const int tensor_index = plane_output_offset
                                         + indexMapper.mapGpuOutputKernelToTensorOutputOffset(i + first_x, j + first_y);
                buffer[tensor_index] = result;
            }
        }
        ...
        for (int k = threadIdx.z; k < num_z_input; k += blockDim.z)
        {
            for (int j = threadIdx.y; j < num_y_input; j += blockDim.y)
            {
                for (int i = threadIdx.x; i < num_x_input; i += blockDim.x)
                {
                    const int tensor_index
                        = plane_input_offset
                          + indexMapper.mapGpuInputKernelToTensorInputOffset(i + first_x, j + first_y, k + first_z);
        ...
        for (int k = threadIdx.z; k < num_z_output; k += blockDim.z)
        {
            for (int j = threadIdx.y; j < num_y_output; j += blockDim.y)
            {
                for (int i = threadIdx.x; i < num_x_output; i += blockDim.x)
                {
                    ...
                    const int tensor_index
                        = plane_output_offset
                          + indexMapper.mapGpuOutputKernelToTensorOutputOffset(i + first_x, j + first_y, k + first_z);
                    buffer[tensor_index] = result;
                }
            }
        }
...
struct TensorEvaluator<const TensorConvolutionOp<Indices, InputArgType, KernelArgType>, GpuDevice>
{
    ...
    TensorEvaluator(const XprType& op, const GpuDevice& device)
        : m_inputImpl(op.inputExpression(), device), m_kernelImpl(op.kernelExpression(), device),
          m_kernelArg(op.kernelExpression()), m_indices(op.indices()), m_buf(NULL), m_kernel(NULL),
    ...
    void executeEval(Scalar* data) const
    {
        ...
        case 1:
        {
            ...
            // " num_blocks.x: " << num_blocks.x << " num_blocks.y: " << num_blocks.y << " maxX: " << maxX << "
        ...
        case 2:
        {
            ...
            // " block_size.z: " << block_size.z << " num_blocks.x: " << num_blocks.x << " num_blocks.y: " <<
            // num_blocks.y << " num_blocks.z: " << num_blocks.z << " maxX: " << maxX << " maxY: " << maxY << " maxP: "
            // << maxP << " shared_mem: " << shared_mem << " in stream " << m_device.stream() << endl;
        ...
        case 3:
        {
            ...
            const int maxX = numext::mini<int>(
                128,
                numext::mini<int>(maxSharedMem / (sizeof(Scalar) * kernel_size_y * kernel_size_z) - kernel_size_x + 1,
                                  numX));
            const int maxY = numext::mini<int>(
                128,
            ...
            // " block_size.z: " << block_size.z << " num_blocks.x: " << num_blocks.x << " num_blocks.y: " <<
            // num_blocks.y << " num_blocks.z: " << num_blocks.z  << " shared_mem: " << shared_mem << " in stream " <<
