...
class QueueInterface
{
  public:
    ...
    get(TensorSycl::internal::RangeAccess<cl::sycl::access::mode::read_write, data_t> data) const
    {
    ...
    memcpyHostToDevice(void* dst, const void* src, size_t n, std::function<void()> callback) const
    {
    ...
    /// function is blocking.
    EIGEN_STRONG_INLINE void
    memcpyDeviceToHost(void* dst, const void* src, size_t n, std::function<void()> callback) const
    {
    ...
    template <typename OutScalar,
              ...
              typename Index,
              typename... T>
    EIGEN_ALWAYS_INLINE void binary_kernel_launcher(
        const Lhs& lhs, const Rhs& rhs, OutPtr outptr, Range thread_range, Index scratchSize, T... var) const
    {
    ...
    unary_kernel_launcher(const InPtr& inptr, OutPtr& outptr, Range thread_range, Index scratchSize, T... var) const
    {
        auto kernel_functor = [=](cl::sycl::handler& cgh)
        {
            // binding the placeholder accessors to a commandgroup handler
    ...
    EIGEN_ALWAYS_INLINE void
    nullary_kernel_launcher(const InPtr& inptr, Range thread_range, Index scratchSize, T... var) const
    {
        auto kernel_functor = [=](cl::sycl::handler& cgh)
        {
            // binding the placeholder accessors to a commandgroup handler
...
struct SyclDevice : public SyclDeviceBase
{
    ...
    template <typename Index>
    EIGEN_STRONG_INLINE void
    memcpyHostToDevice(Index* dst, const Index* src, size_t n, std::function<void()> callback = {}) const
    {
    ...
    template <typename Index>
    EIGEN_STRONG_INLINE void
    memcpyDeviceToHost(void* dst, const Index* src, size_t n, std::function<void()> callback = {}) const
    {
