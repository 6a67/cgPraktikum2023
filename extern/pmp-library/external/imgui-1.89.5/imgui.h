...
BeginPopupContextItem(const char* str_id = NULL,
                      ...
                      = 1); // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to
                            // previous item. If you want to use that on a non-interactive item such as Text() you need
...
IMGUI_API bool BeginDragDropSource(ImGuiDragDropFlags flags
                                   = 0); // call after submitting an item which may be dragged. when this return true,
...
struct ImGuiIO
{
    ...
    IMGUI_API void SetKeyEventNativeData(ImGuiKey key,
                                         ...
                                         = -1); // [Optional] Specify index for legacy <1.87 IsKeyXXX() functions with
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont*
    AddFontFromMemoryTTF(void* font_data,
                         int font_size,
                         float size_pixels,
                         const ImFontConfig* font_cfg = NULL,
...
struct ImFont
{
    ...
    IMGUI_API void AddRemapChar(ImWchar dst,
                                ImWchar src,
                                bool overwrite_dst
                                = true); // Makes 'dst' character/glyph points to 'src' character/glyph. Currently needs
