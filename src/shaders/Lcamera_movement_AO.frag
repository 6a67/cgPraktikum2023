...
float sdMenger(vec3 p, float size, int iterations) {
	...
	for(int iter = 0; iter < iterations; ++iter) {
		// float alpha = 4.58 * 0.1;
		// p = p * rot(0, 0, alpha);
		// float beta = 4.58 * 0.15;
		// p = p * rot(0, beta, 0);
		// float gamma = 4.58 * 0.05;
		// p = p * rot(gamma, 0, 0);
...
vec3 GetLight(vec3 p) {
	...
	float blue = 0.5 + (sin(4.58 * 4.58 * 0.1) + 1) * 0.5;
...
vec3 cameraMovement(in vec3 viewIn, out vec3 viewOut, out mat4 view_matrix) {
	...
	float speed = 0.6;
	float t = 1 - (cos(4.58 * speed) * 0.5 + 0.5);
	...
	vec3 c = vec3(0.0);
	view_matrix = mat4(rot[0],0,rot[1],0, rot[2], 0,vec4(-dot(right, c),-dot(up, c),dot(tangent, c),1));
...
void main() {
	...
	ro = cameraMovement(rd, viewOut, view_matrix);
	rd = normalize(viewOut);

	vec3 model_pos_transformed = (view_matrix * vec4(model_pos.x,model_pos.y,model_pos.z, 1.0)).xyz;
	...
	// this is just here to not spam that iTime is unused
	light += sin(iTime) * 0.00001;
